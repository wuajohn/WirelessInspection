...
/*
+ (NSArray *)latestGeoreferencedPhotos//获取一堆照片
{
    NSString *request = [NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.search&license=1,2,4,7&has_geo=1&extras=original_format,description,geo,date_upload,owner_name"];
    return [[self executeFlickrFetch:request] valueForKeyPath:@"photos.photo"];
}
*/

+ (NSArray *)latestGeoreferencedPhotos//测试从文件中获取
{
    //NSData *jsonData = [NSData dataWithContentsOfFile:@"latestGeoreferencedPhotos.geojson"];
    NSString* filePath = [[NSBundle mainBundle] pathForResource:@"latestGeoreferencedPhotos.geojson" ofType:nil] ;
    ...
    return [results valueForKeyPath:@"photos.photo"];
}
...
+ (NSURL *)urlForPhoto:(NSDictionary *)photo format:(FlickrPhotoFormat)format//解析一张照片
{
